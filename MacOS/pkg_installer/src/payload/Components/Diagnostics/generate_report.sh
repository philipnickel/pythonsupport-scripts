...
# Function to auto-discover diagnostic components dynamically
discover_components() {
    echo -e "${BLUE}Dynamically discovering diagnostic components...${NC}" >&2
    
    local -a component_definitions=()
    local components_base_dir="MacOS/Components/Diagnostics/Components"
    
    # Function to discover components recursively
    discover_components_recursive() {
        local current_path="$1"
        local category="$2"
        local subcategory="$3"
        
        # Try to get directory listing from repository
        local temp_listing="$TEMP_DIR/dir_listing_$(echo "$current_path" | tr '/' '_').txt"
        
        # Use GitHub API to list directory contents
        local api_url="https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/contents/${current_path}"
        ...
        local found_branch=""
        for br in "${branches[@]}"; do
            local branch_api_url="https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/contents/${current_path}?ref=${br}"
            if curl -sfL "$branch_api_url" -o "$temp_listing" 2>/dev/null && [[ -s "$temp_listing" ]]; then
                found_branch="$br"
                break
            fi
        done
        ...
        # Parse directory listing and process each item
        while IFS= read -r line; do
            if [[ "$line" =~ \"name\":\"([^\"]+)\" ]]; then
                local item_name="${BASH_REMATCH[1]}"
                local item_type=""
                
                # Extract item type (file or dir)
                ...
                if [[ "$item_type" == "file" && "$item_name" == *.sh ]]; then
                    # Found a .sh file - this is a diagnostic component
                    local script_name="${item_name}"
                    local script_path="$current_path/$script_name"
                    
                    # Download the script temporarily to extract metadata
                    local temp_script="$TEMP_DIR/meta_${script_name}"
                    if download_repo_file "$script_path" "$temp_script"; then
                        # Extract metadata from the script
                        local display_name=$(extract_metadata "$temp_script" "name")
                        local script_category=$(extract_metadata "$temp_script" "category")
                        local script_subcategory=$(extract_metadata "$temp_script" "subcategory")
                        
                        # Use extracted metadata if available, otherwise infer from path
                        local final_category="${script_category:-$category}"
                        local final_subcategory="${script_subcategory:-$subcategory}"
                        local final_display_name="${display_name:-${script_name%.sh}}"
                        
                        # Add to component definitions
                        # Format: "Category:Subcategory:script_name:Display Name:repo_path"
                        component_definitions+=("${final_category}:${final_subcategory}:${script_name}:${final_display_name}:${script_path}")
                        
                        echo "    Found: $final_category > $final_subcategory → $final_display_name" >&2
                    fi
                    
                elif [[ "$item_type" == "dir" ]]; then
                    # Found a directory - recurse into it
                    local new_path="$current_path/$item_name"
                    local new_category="$category"
                    local new_subcategory="$subcategory"
                    
                    # Determine category and subcategory based on depth
                    if [[ -z "$category" ]]; then
                        # First level directory becomes the category
                        new_category="$item_name"
                    elif [[ -z "$subcategory" ]]; then
                        # Second level directory becomes the subcategory
                        new_subcategory="$item_name"
                    else
                        # Deeper levels extend the subcategory
                        new_subcategory="${subcategory}/${item_name}"
                    fi
                    
                    # Recursively discover components in subdirectory
                    discover_components_recursive "$new_path" "$new_category" "$new_subcategory"
                fi
            fi
        done < "$temp_listing"
    }
    ...
    if [[ "${BASH_SOURCE[0]}" == "/dev/stdin" ]] || [[ "${BASH_SOURCE[0]}" == "bash" ]]; then
        # Running remotely - use API discovery
        echo "  Using remote repository discovery..." >&2
        discover_components_recursive "$components_base_dir" "" ""
    else
        # Running locally - can use local filesystem
        echo "  Using local filesystem discovery..." >&2
        local SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
        local components_dir="$SCRIPT_DIR/Components"
        
        if [[ -d "$components_dir" ]]; then
            # Use find to discover all .sh files
            while IFS= read -r -d '' script_file; do
                if [[ -f "$script_file" && "$script_file" == *.sh ]]; then
                    # Extract path components
                    local rel_path="${script_file#$components_dir/}"
                    local script_name=$(basename "$script_file")
                    
                    # Parse directory structure for category/subcategory
                    local path_parts=(${rel_path//\// })
                    local final_category="${path_parts[0]}"
                    local final_subcategory="${path_parts[1]}"
                    
                    # Handle deeper nesting
                    if [[ ${#path_parts[@]} -gt 3 ]]; then
                        final_subcategory="${path_parts[1]}"
                        for ((i=2; i<${#path_parts[@]}-1; i++)); do
                            final_subcategory="${final_subcategory}/${path_parts[i]}"
                        done
                    fi
                    
                    # Extract metadata from the script
                    local display_name=$(extract_metadata "$script_file" "name")
                    local script_category=$(extract_metadata "$script_file" "category")
                    local script_subcategory=$(extract_metadata "$script_file" "subcategory")
                    
                    # Use extracted metadata if available, otherwise infer from path
                    final_category="${script_category:-$final_category}"
                    final_subcategory="${script_subcategory:-$final_subcategory}"
                    local final_display_name="${display_name:-${script_name%.sh}}"
                    
                    # Build relative path for repository access
                    local repo_path="Components/${rel_path}"
                    
                    # Add to component definitions
                    component_definitions+=("${final_category}:${final_subcategory}:${script_name}:${final_display_name}:${repo_path}")
                    
                    echo "    Found: $final_category > $final_subcategory → $final_display_name" >&2
                fi
            done < <(find "$components_dir" -name "*.sh" -type f -print0)
        else
            echo "  Warning: Local Components directory not found, falling back to remote discovery..." >&2
            discover_components_recursive "$components_base_dir" "" ""
        fi
    fi
    
    # Sort component definitions for consistent ordering
    IFS=$'\n' component_definitions=($(sort <<<"${component_definitions[*]}"))
    unset IFS
    ...
    echo "  Dynamically discovered $count diagnostic components" >&2
    echo "" >&2
}
